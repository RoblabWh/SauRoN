...
class A2CAgent:
    ...
    def actor_optimizer(self):
        ...
        pdf = 1. / K.sqrt(2. * np.pi * sigma_sq) * K.exp(-K.square(action - mu) / (2. * sigma_sq)) #DAS KENNE ICH !!!!! (aus dem pytorch yt video zum continious a2c)
...
def unusedForReference(self, author=Christian):
    from keras.backend import mean, squeeze, placeholder
    ...
    neglogp = self.neglog_continuous(self._ACTION, self._mu, self._var)
    pg_loss = mean(self._ADVANTAGE * neglogp) # mean from Keras Backend

    value_loss = mean_squared_error(squeeze(self._value, axis=-1), self._REWARD) * self._coefficient_value # value ist Ergebnis vom Critic und coefficient_value ist bei ihm 0.5

    entropy = self.entropy_continuous(self._var) # varianz aus dem Actor
    entropy = mean(entropy) * self._coefficient_entropy # Wert (bei Im 0)
    loss = pg_loss + value_loss - entropy

def neglog_continuous(self, action, mu, var):
    from keras.backend import square, exp, log, cast, shape
